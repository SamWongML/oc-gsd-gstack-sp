...
> The five enforcement layers, each strictly stronger than the previous. AGENTS.md is the softest; `command.execute.before` is the hardest.
...
| Switches to `harness` agent, then calls `/gstack-office-hours` after EXECUTION started | Layer 4 (`command.execute.before` reads HARNESS.md leg=execution; skill is in `Forbidden next`) | Hard abort with redirect: "Allowed-next: ..."  |
...
| Verification leg succeeds but no one taps the next agent | Layer 4b (auto-advance inside `command.execute.before` after the allow check) | Sentinel-command detection (`gsd-verify-work` in `verification`, `gstack-ship` in `ship`) atomically rewrites `Leg:` in `HARNESS.md` and breadcrumbs the transition |
...
- Layer 4: <5 ms per gsd-*/gstack-* slash command (one file read, one set lookup in `command.execute.before`).
- Layer 4b: <5 ms on the same hook after the allow check (sentinel command match → atomic `Leg:` rewrite only on transition).
